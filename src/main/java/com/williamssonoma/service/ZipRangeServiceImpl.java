...
/**
 * This is the business service implementation class used to minimize the zip
 * code range.
 * 
 * @author Nelson Pinto
 *
...
public class ZipRangeServiceImpl implements ZipRangeService {
	...
	public String minimizeZipRange(String[] zipRanges) throws ZipRangeMinimizerException {
		...
		// counter for tracking number of times the zip code is found in the input
		// range.
		int occurences = 0;
		...
		for (Integer zipCode : zipCodes) {
			...
			for (ZipRange zipRange : zipRangeList) {
				if (zipCode >= zipRange.getLowZipCode() && zipCode <= zipRange.getHighZipCode()) {
					occurences++;
				}
			}
			...
			// If zipRangeCounter is 2. then the range is closed and space is added for the
			// next range.
